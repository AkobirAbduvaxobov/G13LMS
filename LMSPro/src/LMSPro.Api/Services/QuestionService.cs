...
public class QuestionService : IQuestionService
{
    ...
    public async Task<PaginatedQuestionDto> GetAllAsync(int skip = 0, int take = 20)
    {
        ...
        string cacheKey = string.Format(CacheKeys.QuestionsAll, skip, take);

        if (MemoryCache.TryGetValue(cacheKey, out PaginatedQuestionDto? cachedQuestions))
        {
        ...
        MemoryCache.Set(cacheKey, result, GetQuestionCacheOptions());
    ...
    public async Task<QuestionGetDto> GetByIdAsync(long questionId)
    {
        ...
        if (MemoryCache.TryGetValue(cacheKey, out QuestionGetDto? cachedQuestion))
        {
    ...
    {
        string defaultAllKey = string.Format(CacheKeys.QuestionsAll, 0, 20);
        MemoryCache.Remove(defaultAllKey);
